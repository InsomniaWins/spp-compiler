...
public class OperationMultiply extends SyntaxTreeNode {
	...
	public OperationMultiply(SyntaxTreeNode left, SyntaxTreeNode right) {
		this.left = left;
		this.right = right;
	}
	...
	public SyntaxTreeNode evaluate() {
		...
		if (!(leftEvaluation instanceof LiteralInteger) || !(rightEvaluation instanceof LiteralInteger)) {
			throw new RuntimeException("Tried to multiply " + leftEvaluation + " with " + rightEvaluation + "!\nThese must be int literals!");
		}
