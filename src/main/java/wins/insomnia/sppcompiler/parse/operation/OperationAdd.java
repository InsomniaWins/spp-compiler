...
public class OperationAdd extends SyntaxTreeNode {
	...
	public OperationAdd(SyntaxTreeNode left, SyntaxTreeNode right) {
		this.left = left;
		this.right = right;
	}
	...
	public SyntaxTreeNode evaluate() {
		...
		if (!(leftEvaluation instanceof LiteralInteger) || !(rightEvaluation instanceof LiteralInteger)) {
			throw new RuntimeException("Tried to add " + leftEvaluation + " with " + rightEvaluation + "!\nThese must be int literals!");
		}
